...
public class LowPointFinder {
    ...
    public static int[] moveLower(Map map, boolean[][] travelled, MyNode location) {
    ...
    public static int[] findLowest(Map map, int iRow, int iColumn) {
        ...
        // somewhat modified version of dijkstra's algorithm
        // initilized to false
        boolean[][] travelled = new boolean[map.getNumRows()][map.getNumColumns()];

        // initial starting location
        int[] startLocation = { iRow, iColumn, map.getAltitude(iRow, iColumn) };

        // uses a tree/graph structure to track diverging paths
        MyNode Locations = new MyNode(startLocation);
        System.out.print("Start Location R");
        System.out.print(Locations.getData()[0] + " C");
        System.out.print(Locations.getData()[1] + " A");
        System.out.println(Locations.getData()[2]);

        // uses driver function to find lower altitudes
        int[] result = moveLower(map, travelled, Locations);
        return result;
    }
