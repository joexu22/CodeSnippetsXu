...
public class LowPointFinder {
    ...
    public static int[] moveLower(Map map, int iRow, int iColumn, boolean[][] travelled, Set<int[]> lowestLocations) {
        ...
        int originalSize = lowestLocations.size();

        // Test UP, LEFT, DOWN, RIGHT to see if adjacent location has lower altitude
        findPath(map, iRow-1, iColumn, travelled, lowestLocations);
        findPath(map, iRow+1, iColumn, travelled, lowestLocations);
        findPath(map, iRow, iColumn-1, travelled, lowestLocations);
        findPath(map, iRow, iColumn+1, travelled, lowestLocations);
        System.out.println("Size of HashSet: "+lowestLocations.size());
        
        int newSize = lowestLocations.size();

        int newLowestAltitude = Integer.MAX_VALUE;
        for (int[] location : lowestLocations) {
            newLowestAltitude = Math.min(lowestAltitude, location[2]);
        }
        if (lowestAltitude == newLowestAltitude && originalSize == newSize) {
        ...
        if (newSize == 1) {
            ...
            return moveLower(map, nextRow, nextCol, travelled, lowestLocations);
        }

        if (newSize > 1) {
            int[][] toIterate = lowestLocations.toArray(new int[lowestLocations.size()][3]);
            for (int[] item : toIterate) {
